...
发布入口是 `.github/workflows/release.yml`。它只响应与应用版本完全一致的语义版本 tag，例如当前 `src-tauri/tauri.conf.json` 为 `0.1.2` 时只能使用 `v0.1.2`。工作流在 Linux x64、Windows x64、macOS Apple Silicon 和 macOS Intel 上构建原生安装包，并始终创建 GitHub **Draft Release**，不会自动公开。
