...
- WebView 只持有 connection id；密码、token 和私钥不得写入前端存储或日志。
...
- 保留协议原生入口：etcd lease/transaction、ZooKeeper ACL/ephemeral、Nacos service/instance/history。
...
正式版本完成的证据必须同时包含：公共命令测试、三种真实服务集成测试、前端生产构建、Tauri release 构建，以及对应平台的安装与安全验证记录。
