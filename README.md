...
- 已实现：方案 A 的 React 页面、Tauri 工程、连接的新建/编辑/复制/删除/测试、非敏感配置迁移、系统凭据库、三协议认证与 TLS、长生命周期会话、可取消的分页/懒加载浏览、资源读取、实时监听、有界标识搜索、精确定位、本地脱敏审计历史、Nacos 2.x/3.x 服务端配置历史、etcd Lease 与 ZooKeeper ACL 只读检查、结构化错误、二进制安全展示、1 MiB 大值保护，以及带版本校验、连接名二次确认、影响预览和脱敏审计的创建/更新/删除链路。
...
- 正在推进：etcd Lease 变更/transaction、ZooKeeper ACL 变更/ephemeral、Nacos service/instance 等协议原生写入与管理能力，随后是 ZooKeeper SASL 扩展、真实版本矩阵和生产发布加固。
