...
const source = readFileSync(
  new URL("../src/resourceTree.ts", import.meta.url),
  "utf8",
);
...
test("expanding and collapsing a resource keeps descendants local to their parent", () => {
  const expanded = tree.expandResourceRow([parent], 0, {
    ...
    items: [
      {
        address: { type: "zookeeper", path: "/apps/api" },
        name: "api",
        readable: true,
        hasChildren: false,
      },
  ...
  assert.deepEqual(tree.collapseResourceRow(expanded, 0, parent.node.address), [
...
test("a continuation row is replaced atomically by the next page", () => {
  ...
  const next = tree.replaceContinuationRow(
    rows,
    0,
    tree.pageRows(
      [
        {
          address: { type: "etcd", keyBase64: "YQ==" },
          name: "a",
          readable: true,
          hasChildren: false,
        },
...
test("stale async pages cannot mutate a different row now occupying the index", () => {
  const replacementParent = {
    ...
    node: {
      ...parent.node,
      address: { type: "zookeeper", path: "/other" },
      name: "other",
    },
  };
  const stalePage = {
  ...
  assert.deepEqual(tree.expandResourceRow([replacementParent], 0, stalePage), [
    replacementParent,
  ]);
  ...
  assert.deepEqual(
    tree.replaceContinuationRow(
      [currentMore],
      0,
      tree.pageRows([], 0, root),
      staleMore,
    ),
    [currentMore],
  );
