...
export function NativeInfoDialog({
  ...
  return (
    ...
    >
      <section
        className="dialog native-info-dialog"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">NATIVE RESOURCE INFO</span>
            <h2>{title}</h2>
          </div>
        ...
        {info?.kind === "etcdLease" && (
          <div className="native-stat-grid">
            ...
            <div>
              <span>剩余 TTL</span>
              <strong>{formatDuration(info.remainingTtlSeconds)}</strong>
            </div>
            <div>
              <span>授予 TTL</span>
              <strong>{formatDuration(info.grantedTtlSeconds)}</strong>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="acl-list">
              {info.entries.map((entry, index) => (
                ...
                >
                  <div>
                    <span className="history-kind">{entry.scheme}</span>
                    <b>{entry.id || "（空身份）"}</b>
                  </div>
        ...
        <p className="form-note">
          此入口只读取协议原生元数据，不会续租、撤销 Lease 或修改 ACL。
        </p>
