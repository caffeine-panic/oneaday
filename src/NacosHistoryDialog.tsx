import type { ResourceHistoryDocument, ResourceHistoryEntry } from "./registry";
...
export function NacosHistoryDialog({
  ...
  return (
    ...
    >
      <section
        className="dialog server-history-dialog"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">NACOS SERVER HISTORY</span>
            <h2>{resourceName}</h2>
          </div>
        ...
        {detail ? (
          <div className="history-detail">
            <div className="history-detail-heading">
              <button className="button" disabled={loading} onClick={onBack}>
                ← 返回历史列表
              </button>
              <span>Revision {detail.entry.revisionId}</span>
            </div>
            <HistoryMetadata entry={detail.entry} />
            {detail.value.encoding === "base64" && (
              <div className="binary-warning">
                该历史值不是有效 UTF-8，已用 Base64 无损展示。
              </div>
            )}
            <textarea
              value={detail.value.content}
              readOnly
              spellCheck={false}
            />
            <p className="form-note">
              读取历史详情会显式加载该版本的
              value；当前切片只读，不提供一键恢复，避免把历史查看伪装成无条件覆盖。
            </p>
          </div>
        ) : (
          <div className="server-history-list">
            ...
            {!loading && items.length === 0 && (
              <div className="empty compact">
                <b>服务端没有历史记录</b>
                <span>历史保留窗口由 Nacos 服务端配置决定。</span>
              </div>
            )}
            {loading && (
              <div className="loading-line">正在读取 Nacos 服务端历史…</div>
            )}
          </div>
        )}
        <div className="dialog-actions">
          {loading && (
            <button className="button" onClick={onCancelOperation}>
              取消请求
            </button>
          )}
          {!detail && nextCursor && (
            <button className="button" disabled={loading} onClick={onLoadMore}>
              加载更早版本
            </button>
          )}
          <button
