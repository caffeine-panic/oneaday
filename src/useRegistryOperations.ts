...
export function useRegistryOperations<Scope extends string>(
  ...
  if (tracker.current === null) tracker.current = new OperationTracker(createId);
  const [active, setActive] = useState<Partial<Record<Scope, string>>>({});

  const start = useCallback((scope: Scope) => {
  ...
  const cancel = useCallback(async (scope: Scope) => {
  ...
  const isCurrent = useCallback(
    (scope: Scope, operationId: string) => tracker.current!.isCurrent(scope, operationId),
    [],
  );
