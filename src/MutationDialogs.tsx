...
export function NewResourceDialog({
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={onCancel}>
      <section className="dialog resource-dialog" onMouseDown={(event) => event.stopPropagation()}>
        <div className="dialog-heading">
          <div><span className="eyebrow">CREATE RESOURCE</span><h2>新建资源</h2></div>
          <button className="icon-button" onClick={onCancel}>×</button>
        </div>
        {adapter === "nacos" ? (
          <div className="form-grid equal">
            <label>Group<input autoFocus value={draft.group} onChange={(event) => onChange({ ...draft, group: event.target.value })} placeholder="DEFAULT_GROUP" /></label>
            <label>Data ID<input value={draft.dataId} onChange={(event) => onChange({ ...draft, dataId: event.target.value })} placeholder="application.yaml" /></label>
          </div>
        ) : (
...
export function MutationConfirmationDialog({
  ...
  const nonAtomic = profile.adapter === "nacos" && mutation.operation !== "update";
  const canConfirm = !busy && confirmationText === profile.name;
  const operationLabel = mutation.operation === "create"
    ? "创建"
  ...
  return (
    <div className="dialog-backdrop" onMouseDown={() => { if (!busy) onCancel(); }}>
      <section className="dialog mutation-dialog" onMouseDown={(event) => event.stopPropagation()}>
        <div className="dialog-heading">
          <div><span className="eyebrow">MUTATION REVIEW</span><h2>确认{operationLabel}</h2></div>
        ...
        <div className="impact-grid">
          ...
          <span>目标</span><b>{addressText(mutation.address)}</b>
          <span>版本条件</span><b>{"expectedVersion" in mutation ? mutation.expectedVersion : "必须不存在"}</b>
          <span>一致性</span><b>{nonAtomic ? "检查后变更（非原子）" : "原子条件变更"}</b>
          {"value" in mutation && <><span>新值大小</span><b>{mutationValueSize(mutation).toLocaleString()} B</b></>}
        </div>
        {nonAtomic && <div className="mutation-warning">Nacos 的该操作没有服务端 CAS 条件接口。客户端会先校验版本/不存在状态，但校验与变更之间仍存在竞争窗口。</div>}
        ...
        <p className="form-note">审计日志只保存版本、编码、大小与 SHA-256，不保存资源原文。变更开始前会先落盘 started 事件。</p>
        <div className="dialog-actions">
          <button className="button" onClick={busy ? onCancelOperation : onCancel}>{busy ? "取消请求" : "返回"}</button>
          <button className={`button ${mutation.operation === "delete" ? "danger" : "primary"}`} disabled={!canConfirm} onClick={onConfirm}>{busy ? "正在提交…" : `确认${operationLabel}`}</button>
        </div>
      </section>
    </div>
  );
...
function addressText(address: ResourceAddress) {
  switch (address.type) {
    case "root": return "/";
    case "etcd": return `etcd:${decodeEtcdKey(address.keyBase64)}`;
    case "etcdPrefix": return `etcd-prefix:${address.prefixBase64}`;
...
function mutationValueSize(mutation: Extract<ResourceMutation, { operation: "create" | "update" }>) {
