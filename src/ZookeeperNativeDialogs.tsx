...
type AclDialogProps = {
  ...
  busy: boolean;
  onExecute: (
    action: Extract<ZookeeperNativeAction, { action: "setAcl" }>,
  ) => void;
...
export function ZookeeperAclDialog({
  ...
  const changed =
    Boolean(info) && JSON.stringify(entries) !== JSON.stringify(info?.entries);
  const canSubmit =
    Boolean(info) &&
    changed &&
    !validation &&
  ...
  const togglePermission = (
    index: number,
    permission: ZookeeperAclPermission,
  ) => {
  ...
  return (
    ...
    >
      <section
        className="dialog native-info-dialog zookeeper-acl-dialog"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">ZOOKEEPER ACL</span>
            <h2>ACL 条件变更</h2>
          </div>
        ...
        {loading && !info && (
          <div className="loading-line">正在读取当前 ACL 与 aversion…</div>
        )}
        {info && (
          <div className="native-summary">
            <span>当前 ACL 版本</span>
            <b>{info.aclVersion}</b>
            <small>{info.entries.length} 条规则</small>
          </div>
        )}
        ...
        <div className="impact-grid">
          ...
          <b>
            {document.address.type === "zookeeper"
              ? document.address.path
              : "—"}
          </b>
          <span>一致性</span>
          <b>服务端 aversion 原子条件写</b>
        </div>
              ...
              <div className="form-grid equal">
                <label>
                  ...
                  <input
                    ...
                    disabled={busy}
                    onChange={(event) =>
                      changeEntry(index, { scheme: event.target.value })
                    }
                    placeholder="world / auth / digest"
                  />
                </label>
                <label>
                  Identity
                  <input
                    ...
                    disabled={busy}
                    onChange={(event) =>
                      changeEntry(index, { id: event.target.value })
                    }
                    placeholder="anyone / user:hash"
                  />
                </label>
              </div>
              <div className="acl-permission-editor">
                {permissions.map((permission) => (
                  <label key={permission.id}>
                    <input
                      type="checkbox"
                      checked={entry.permissions.includes(permission.id)}
                      disabled={busy}
        ...
        <button
          className="button"
          disabled={busy || entries.length >= 256}
          onClick={() =>
            setEntries((current) => [
              ...current,
              { scheme: "digest", id: "", permissions: ["read"] },
            ])
          }
        >
          ＋ 添加 ACL 身份
        ...
        {validation && (
          <div className="mutation-warning danger-warning">{validation}</div>
        )}
        <div className="mutation-warning">
          ACL 错配可能立即使当前连接失去访问权。客户端强制保留至少一个 ADMIN
          身份，但无法判断该身份是否属于当前操作者。
        </div>
        <label className="production-confirmation">
          确认以 aversion {info?.aclVersion ?? "—"} 提交，请输入当前连接名{" "}
          <b>{profile.name}</b>。
          <input
            value={confirmation}
            disabled={busy}
        ...
        <p className="form-note">
          审计只记录目标、ACL 条目数和版本，不保存 znode 原文。服务端返回
          BadVersion 时不会覆盖他人修改。
        </p>
  ...
  return (
    ...
    >
      ...
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">ZOOKEEPER NATIVE CREATE</span>
            <h2>确认创建{createModeLabels[action.mode]}</h2>
          </div>
          <button className="icon-button" disabled={busy} onClick={onClose}>
            ×
          </button>
        ...
        <div className="impact-grid">
          ...
          <b>{path}</b>
          <span>节点模式</span>
          <b>{createModeLabels[action.mode]}</b>
          <span>ACL</span>
          <b>继承父节点</b>
          <span>新值大小</span>
          <b>{bytes.toLocaleString()} B</b>
        </div>
        {action.mode !== "persistentSequential" && (
          <div className="mutation-warning">
            临时节点属于当前桌面连接的 ZooKeeper session。断开连接或 session
            过期后，服务端会自动删除它。
          </div>
        )}
        {action.mode.endsWith("Sequential") && (
          <p className="form-note">
            请求路径是前缀；服务端会追加十位或十九位序号，提交后界面将定位到实际路径。
          </p>
        )}
        <label className="production-confirmation">
        ...
        <p className="form-note">
          写入会记录脱敏审计，只保存版本、编码、大小与 SHA-256，不保存资源原文。
        </p>
        <div className="dialog-actions">
          <button
            className="button"
...
function validateAcl(entries: ZookeeperAclEntry[]) {
  ...
  for (const entry of entries) {
    ...
    if (identities.has(identity))
      return `身份 ${scheme}:${id || "（空）"} 重复。`;
    identities.add(identity);
    if (entry.permissions.length === 0)
      return `身份 ${scheme}:${id || "（空）"} 没有任何权限。`;
    hasAdmin ||= entry.permissions.includes("admin");
  }
  return hasAdmin
    ? undefined
    : "至少保留一个“管理 ACL（ADMIN）”身份，避免 ACL 永久失控。";
}
