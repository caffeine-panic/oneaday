...
export function EtcdTransactionDialog({
  ...
  return (
    ...
    >
      <section
        className="dialog etcd-transaction-dialog"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <div className="dialog-heading">
          <div>
            <span className="eyebrow">ETCD TRANSACTION</span>
            <h2>原子批量事务</h2>
          </div>
          ...
          {items.map((item, index) => (
            <section className="transaction-item" key={index}>
              ...
              <div className="form-grid transaction-operation-grid">
                    ...
                    onChange={(event) =>
                      replace(index, {
                        ...
                        operation: event.target
                          .value as EtcdTransactionDraftItem["operation"],
                      })
                    }
                  >
                ...
                <label>
                  ...
                  <input
                    value={item.key}
                    onChange={(event) =>
                      replace(index, { ...item, key: event.target.value })
                    }
              ...
              {item.operation !== "delete" && (
                <>
                  <label className="transaction-encoding">
                    值编码
                    <select
                      value={item.encoding}
                      onChange={(event) =>
                  ...
                  <label>
                    ...
                    <textarea
                      value={item.value}
                      onChange={(event) =>
                        replace(index, { ...item, value: event.target.value })
                      }
                      spellCheck={false}
                    />
                  </label>
        ...
        <div className="mutation-warning">
          同一个 key 不能在一次事务中出现两次。任一 key
          的版本条件不满足时，整个事务都不会执行；提交结果未知时请先刷新，切勿直接重试。
        </div>
        <label className="production-confirmation">
          请输入当前连接名 <b>{profile.name}</b> 确认一次性提交全部操作。
          <input
        ...
        <p className="form-note">
          事务中每个资源都会分别写入 started/applied
          脱敏审计，只记录版本、大小、编码和 SHA-256。
        </p>
